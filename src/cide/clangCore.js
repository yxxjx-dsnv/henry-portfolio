...
// Vendored from binji/wasm-clang (Apache-2.0) and patched for henrykim.ca:
//  - compiles C (-x c) instead of C++, and links multiple objects
//  - App.run records exitCode instead of throwing on a normal non-zero exit
//  - clock_time_get implemented (binji threw NotImplemented)
//  - added API.compileLinkRunMulti(files); exported as an ES module
// See docs/superpowers/specs/2026-07-21-aps105-labs-ide-design.md
...
class App {
  ...
  async run() {
    ...
    try {
      ...
      if (exn instanceof ProcExit) {
        this.exitCode = exn.code;
        if (exn.code === RAF_PROC_EXIT_CODE) return true;
        this.allowRequestAnimationFrame = false;
        return false;  // normal exit (0 or non-zero) — caller reads exitCode
      }
      // A real trap (e.g. out-of-bounds, stack overflow): surface + rethrow.
      this.exitCode = 134;
      this.memfs.hostWrite(`\nruntime error: ${exn.message}\n`);
      throw exn;
    }
  }
  ...
  clock_time_get(clock_id, precision, time_out) {
    ...
    this.mem.write64(time_out, Number(ns & 0xffffffffn),
...
class API {
  ...
  // Compile every .c in `files` to an object, link them into one wasm module,
  // and run it. `.h` files are written to the FS so #include resolves. Reads
  // program output via hostWrite while this._phase === 'run'. Returns the exit
  // code and the stage reached (so the caller can show compile diagnostics).
  async compileLinkRunMulti(files) {
    await this.ready;
    const cArgs = [
      ...
      '-fno-color-diagnostics', '-I', '.',
    ];
    const clang = await this.getModule(this.clangFilename);
    const objs = [];
    for (const f of files) this.memfs.addFile(f.name, f.contents);
    for (const f of files) {
      if (!f.name.endsWith('.c')) continue;
      const obj = f.name.replace(/\.c$/, '.o');
      const app = await this.run(clang, 'clang', '-cc1', '-emit-obj',
          ...cArgs, '-O2', '-o', obj, '-x', 'c', f.name);
      if (app.exitCode !== 0) return {exitCode: app.exitCode, stage: 'compile'};
      objs.push(obj);
    }
    const lld = await this.getModule(this.lldFilename);
    const wasm = 'prog.wasm';
    const libdir = 'lib/wasm32-wasi';
    const lapp = await this.run(lld, 'wasm-ld', '--no-threads',
        '--export-dynamic', '-z', 'stack-size=1048576', `-L${libdir}`,
        `${libdir}/crt1.o`, ...objs,
        '-lc', '-lc++', '-lc++abi', '-lcanvas', '-o', wasm);
    if (lapp.exitCode !== 0) return {exitCode: lapp.exitCode, stage: 'link'};
    const buffer = this.memfs.getFileContents(wasm);
    const mod = await WebAssembly.compile(buffer);
    this._phase = 'run';
    const app = await this.run(mod, wasm);
    return {exitCode: app.exitCode, stage: 'run'};
  }
}

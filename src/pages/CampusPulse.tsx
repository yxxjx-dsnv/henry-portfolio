      ...
      <section className="essay-section">
        <div className="text">
          ...
          >
            ...
            <p>{t("The application form asked why I wanted to attend, and my honest answer was that small problems in campus life had been bothering me for a while. Information is scattered across a handful of channels: department websites, email newsletters, Quercus announcements, club social media. More than once I learned about a lecture I wanted only after its registration had closed. The systems make students do the searching that software should be doing. I wrote that I wanted to move beyond passively observing those problems and try to analyze and redesign the system underneath one of them.")}</p>
            <br />
            ...
            <p>{t("The problem we picked was not original, but it was real, and all four of us had lived it. During midterm season you walk to Robarts, take the elevator up, walk the floor, find nothing, and repeat the loop in the next building. Thousands of students hunt for seats in parallel, and there is no live signal telling you where not to bother going. Every student knows this feeling, but there was no data anyone could check.")}</p>
            <br />
            <p>{t("Deciding how to measure occupancy turned out to be the most instructive part of the day. Our first idea was counting devices on campus Wi-Fi. It sounded clever until we said \"track everyone's phones\" out loud, and we dropped it. The second was T-card tap data, which fails on coverage: only a few buildings, like Robarts and Gerstein, have T-card gates at all, so the signal could never cover the whole campus. What survived was the simplest idea: point a camera at a study space, count the people in frame, and compare the count to the floor's seat capacity. It measures exactly the thing a student cares about, which is seats. In the end, the constraints made the decision for us, and going through that elimination taught me a lot about how engineering choices are really made.")}</p>
            <br />
            ...
            <Figure
              ...
              height={1117}
            />
            <p>{t("The dashboard kept things simple and clear: a percentage for every floor, low / mid / high color bands, a \"best location right now\" answer at the top, a daily trend line, and building alerts for things like a broken elevator. When we presented, I opened the architecture walkthrough for the judges and explained our pipeline as simply as I could, even though I had only learned these services that morning.")}</p>
            <br />
            <p>{t("Rather than only describe it, here is the dashboard itself, ported from our React source and running on the demo feed we presented. Search or sort the directory, open any building for its floors and daily trend, flip between cards and table, and watch the occupancy refresh every few seconds.")}</p>
            <br />
            <CampusPulseDash />
            <p className="doc-note">
              {t('A self-contained reproduction of our React dashboard on the presentation data; the real one read live counts from the AWS pipeline.')}
            </p>
            <figure className="story-figure">
              <AutoVideo
                src={`${MEDIA}/demo.mp4`}
            ...
            <p>{t("The concrete lessons were about AWS. I learned that Rekognition lets you use serious computer vision without training or hosting your own model. I learned how DynamoDB stores and serves the readings, how Lambda and API Gateway turn a function into an API, and how a static site on S3 becomes a URL anyone can open. In one focused day, I went from never having used AWS to having our service live on a public URL, and that changed my sense of what a single day of work can do.")}</p>
            <br />
            <p>{t("I also learned something about being honest in design. Twice that day, we gave up an idea that sounded impressive for one that worked: first when we chose the sensor, and again when we trimmed the features down to what could be real by 5 p.m. I try to remember that whenever one of my projects starts becoming too ambitious.")}</p>
            <br />
            <p className="story-head">Where this goes</p>
            <p>{t("Campus Pulse has an obvious roadmap: room booking on top of the live map, occupancy prediction once the readings accumulate into history, richer alerts, more buildings. But for me the project matters mostly as a first proof of a bigger belief I wrote into that application: campus information lives in fragments, and it should feel like one system. I had even sketched what I wanted in the application, one place where every campus event can be found and joined in a click, synced to your calendar, with deadlines that reach you instead of waiting to be found. Campus Pulse is the first piece of that idea I have built and put on the internet, and I want to keep building the next pieces the same way.")}</p>
            <br />
            <Figure
              ...
              height={1600}
            />
            <p>{t("On the application I promised that I would learn quickly. Leaving at five o'clock, I felt I had kept that promise once, and I want to keep testing it.")}</p>
            <br />

...
const ko: Record<string, string> = {
  ...
  'The application form asked why I wanted to attend. I wrote that small problems in campus life had been bothering me for a while. Information is scattered across department websites, email newsletters, Quercus announcements and club social media, and more than once I found out about a lecture I wanted only after registration had closed. Students end up doing the searching that software should do for them. I wanted to stop just noticing that and try to analyze and redesign the system under one of those problems.':
    '지원서에는 참가하고 싶은 이유를 묻는 항목이 있었습니다. 저는 캠퍼스 생활의 작은 문제들이 꽤 오래전부터 신경 쓰였다고 썼습니다. 정보는 학과 웹사이트, 이메일 뉴스레터, Quercus 공지, 동아리 소셜 미디어에 흩어져 있고, 듣고 싶던 강연을 등록 마감이 지난 뒤에야 알게 된 적이 한두 번이 아니었습니다. 소프트웨어가 대신 해 줘야 할 검색을 결국 학생이 하고 있는 셈입니다. 그저 그런 문제를 알아채는 데서 그치지 않고 그중 하나의 밑바탕에 있는 시스템을 분석하고 다시 설계해 보고 싶었습니다.',
  ...
  'The problem we picked was an ordinary one, and all four of us had lived it. During midterm season you walk to Robarts, take the elevator up, walk the floor, find nothing, and repeat the loop in the next building. Thousands of students hunt for seats at the same time, and there is no live signal telling you where not to bother going.':
    '저희가 고른 문제는 평범한 것이었고 네 명 모두 직접 겪은 일이었습니다. 중간고사 기간이면 Robarts까지 걸어가서 엘리베이터를 타고 올라가 한 층을 다 돌아도 자리가 없고 다음 건물에서 같은 과정을 반복합니다. 수천 명의 학생이 동시에 자리를 찾아다니는데 어디는 가 봐야 소용없다고 알려 주는 실시간 신호는 없습니다.',
  'Deciding how to measure occupancy was the part of the day I learned the most from. Our first idea was counting devices on campus Wi-Fi. It sounded clever until we said "track everyone\'s phones" out loud, and we dropped it. The second was T-card tap data, which fails on coverage: only a few buildings, like Robarts and Gerstein, have T-card gates at all, so it could never cover the whole campus. What survived was the simplest idea: point a camera at a study space, count the people in frame, and compare the count to the floor\'s seat capacity. It measures the thing a student actually cares about, which is seats.':
    '점유율을 어떻게 측정할지 정하는 일이 그날 가장 많이 배운 부분이었습니다. 첫 번째 아이디어는 캠퍼스 Wi-Fi에 접속한 기기 수를 세는 것이었습니다. 그럴듯하게 들렸지만 "모든 사람의 휴대폰을 추적한다"고 소리 내어 말해 보는 순간 접었습니다. 두 번째는 T-card 태그 데이터였는데 범위가 문제였습니다. T-card 게이트가 있는 건물은 Robarts와 Gerstein 등 몇 곳뿐이라 캠퍼스 전체를 결코 덮을 수 없었습니다. 살아남은 것은 가장 단순한 아이디어였습니다. 학습 공간에 카메라를 두고 화면 안의 사람 수를 세어 그 층의 좌석 수와 비교하는 것입니다. 학생이 정말 궁금해하는 것, 즉 자리를 측정합니다.',
  ...
  'The detector on the busiest feed, gerstein-library#2F, boxing and counting 23 people at once.':
    '가장 붐비는 영상 gerstein-library#2F에서 동작하는 감지기. 한 번에 23명을 박스로 표시하고 셉니다.',
  ...
  'And the sidney-smith#2F feed.': '그리고 sidney-smith#2F 영상.',
  ...
  'The dashboard shows a percentage for every floor, low / mid / high color bands, a "best location right now" answer at the top, a daily trend line, and building alerts for things like a broken elevator. When we presented, I did the architecture walkthrough for the judges and explained our pipeline as simply as I could, having learned these services that morning.':
    '대시보드에는 층마다 퍼센트, 낮음 / 보통 / 높음 색상 구간, 상단의 "지금 가장 좋은 장소" 답, 일간 추이 선, 엘리베이터 고장 같은 건물 알림이 표시됩니다. 발표 때는 제가 심사위원들 앞에서 아키텍처 설명을 맡았습니다. 그날 아침에야 배운 서비스들이었지만 저희 파이프라인을 최대한 쉽게 풀어 설명했습니다.',
  'The dashboard itself is below, ported from our React source. You can search or sort the directory, open a building for its floors and daily trend, switch between cards and table, and watch the occupancy refresh every few seconds.':
    '아래에 대시보드를 그대로 두었습니다. 저희 React 소스를 옮겨 온 것입니다. 목록을 검색하거나 정렬하고 건물을 열어 층별 현황과 일간 추이를 볼 수 있습니다. 카드와 표를 오가며 몇 초마다 점유율이 갱신되는 모습도 확인할 수 있습니다.',
  'This copy runs on the presentation data; the real one read live counts from the AWS pipeline.': '이 재현본은 발표 데이터로 동작합니다. 실제 대시보드는 AWS 파이프라인에서 실시간 인원수를 읽었습니다.',
  ...
  'The slides, with a note under each on what it covered and who presented it.': '슬라이드입니다. 각 슬라이드 아래에 어떤 내용을 다뤘고 누가 발표했는지 메모를 달았습니다.',
  ...
  'What students see: real-time building density, floor-by-floor occupancy in percentages and colour, the least-crowded facility, and a crowd trend over time.':
    '학생이 보는 화면: 실시간 건물 밀도, 퍼센트와 색상으로 표시한 층별 점유율, 가장 한산한 시설, 시간에 따른 혼잡도 추이.',
  ...
  'Thank you.': '감사합니다.',
  'The code behind the system opens here too:': '시스템 뒤의 코드도 여기서 열어 볼 수 있습니다:',
  // the code shelf
  ...
  'The team': '팀',
  'Campus Pulse was built by four people who had mostly just met: Joshua, Suyeon, Byeongmin, and me, registered under the team code 6SIX7. We split the work roughly along the pipeline. Detection and the AWS setup went to one side, the dashboard and its data to the other, and we merged everything in the final hours. Building MONO had taught me the hard way to break a problem into smaller units, put structure ahead of speed, and test while building instead of after. We spent the first half hour agreeing on what we would not build, which paid off at the end of the day when everything had to be merged at once.':
    'Campus Pulse는 대부분 그날 처음 만난 네 사람이 만들었습니다. Joshua, Suyeon, Byeongmin과 저, 팀 코드 6SIX7로 등록했습니다. 일은 대략 파이프라인을 따라 나눴습니다. 감지와 AWS 설정은 한쪽이, 대시보드와 그 데이터는 다른 쪽이 맡았습니다. 마지막 몇 시간 동안 모든 것을 합쳤습니다. MONO를 만들며 어렵게 배운 것이 있었습니다. 문제는 작은 단위로 나눌 것, 속도보다 구조를 앞세울 것, 다 만든 뒤가 아니라 만들면서 테스트할 것. 저희는 첫 30분을 무엇을 만들지 않을지 합의하는 데 썼고, 모든 것을 한꺼번에 합쳐야 했던 하루의 마지막에 그 덕을 봤습니다.',
  'The four teammates posing around a large AWS cloud-logo sign in front of a wall patterned with binary digits.':
    '이진수 패턴 벽 앞의 커다란 AWS 구름 로고 간판 주위에서 포즈를 취한 네 명의 팀원.',
  'Team 6SIX7, in front of the AWS sign.': 'AWS 간판 앞의 6SIX7 팀.',
  'What I learned': '배운 것',
  'The concrete lessons were about AWS. Rekognition gives you working computer vision without training or hosting a model of your own. DynamoDB stores and serves the readings, Lambda and API Gateway turn a function into an API, and a static site on S3 becomes a URL anyone can open. I had never used AWS before that morning, and by the afternoon our service was on a public URL.':
    '구체적인 교훈은 AWS에 관한 것이었습니다. Rekognition을 쓰면 모델을 직접 학습시키거나 호스팅하지 않고도 동작하는 컴퓨터 비전을 쓸 수 있습니다. DynamoDB가 측정값을 저장하고 제공하고, Lambda와 API Gateway가 함수 하나를 API로 만들고, S3의 정적 사이트는 누구나 열 수 있는 URL이 됩니다. 그날 아침까지 AWS를 한 번도 써 본 적이 없었는데 오후에는 저희 서비스가 공개 URL에 올라가 있었습니다.',
  'The other lesson was about being honest in design. Twice that day we gave up an idea that sounded impressive for one that would work: when we chose the sensor, and again when we cut the features down to what could be real by 5 p.m. I try to remember that whenever one of my projects starts getting too ambitious.':
    '다른 교훈은 설계에서 솔직해지는 것이었습니다. 그날 저희는 두 번, 그럴듯하게 들리는 아이디어를 버리고 실제로 동작할 쪽을 택했습니다. 센서를 고를 때, 그리고 오후 5시까지 실제로 만들 수 있는 것만 남기고 기능을 덜어낼 때였습니다. 제 프로젝트가 지나치게 야심 차지기 시작할 때마다 그 일을 떠올리려고 합니다.',
  'Where this goes': '앞으로의 방향',
  'The obvious next steps are room booking on top of the live map, occupancy prediction once the readings build up a history, better alerts, and more buildings. What I care about more is the idea I put in the application: campus information is scattered, and it should work like one system. I had sketched it there, one place where every campus event can be found and joined in a click, synced to your calendar, with deadlines that come to you. Campus Pulse is the first piece of that I have built and put on the internet.':
    '다음 단계는 분명합니다. 실시간 지도 위의 스터디룸 예약, 측정값이 이력으로 쌓이면 가능해질 점유율 예측, 더 나은 알림, 더 많은 건물. 제가 더 중요하게 여기는 것은 지원서에 적었던 생각입니다. 캠퍼스 정보는 흩어져 있고 하나의 시스템처럼 동작해야 한다는 것입니다. 지원서에는 그 모습까지 그려 두었습니다. 캠퍼스의 모든 행사를 한곳에서 찾아 클릭 한 번으로 참여하고 캘린더에 동기화되며 마감일이 먼저 찾아오는 곳입니다. Campus Pulse는 그중 제가 처음으로 만들어 인터넷에 올린 조각입니다.',
  ...
  "On the application I promised that I would learn quickly. Leaving at five o'clock, I felt I had kept that promise at least once.":
    '지원서에서 저는 빠르게 배우겠다고 약속했습니다. 5시에 나서면서 그 약속을 적어도 한 번은 지켰다고 느꼈습니다.',
  ...
  'These are Sidney Smith Commons hours on the ground floor, not necessarily the full building.':
    '이 시간은 1층 Sidney Smith Commons의 운영 시간이며 건물 전체에 해당하지 않을 수 있습니다.',
};

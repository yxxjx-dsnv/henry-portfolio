...
def plot_fit(my_func, xdata, ydata, xerror=None, yerror=None, init_guess=None, font_size=14,
            ...
            title="Period vs. Angle of Pendulum Graph"):    
    ...
    # 각 파라미터에 의미 라벨 부여 (필요 시 수정 가능)
    labels = ["C (quadratic term)", "B (linear term)", "T₀ (constant term)"]

    for i in range(len(popt)):
        label = labels[i] if i < len(labels) else f"Parameter {i+1}"
        print(f"{label}: {popt[i]:.9f} ± {puncert[i]:.9f}")
    ...
    plt.savefig("Period vs. Angle of Pendulum Graph.pdf", format="pdf", bbox_inches='tight')
